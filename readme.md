...
O Laravel utiliza o arquivo *.env* para guardar as configurações de ambiente. Como você pode notar no nosso exemplo, as configurações de banco de dados e o nome do ambiente no arquivo versionado estão com variáveis, estas serão substituidas por valores configurados em cada etapa no próprio [Snap CI](https://docs.snap-ci.com/pipeline/) através no nosso script *alterar_variaveis.sh*.
